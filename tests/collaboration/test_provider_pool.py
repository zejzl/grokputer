...
def register_test_provider(registry: ProviderRegistry, instance: ProviderInstance) -> bool:
    """Register a test provider instance through the registry."""
    return registry.register_provider(
        provider_id=instance.metadata.name,
...
async def test_acquire_connection_circuit_breaker_open():
    ...
    circuit_breaker.last_failure_time = time.time()  # Freshly opened, still in recovery timeout
...
def test_select_provider_round_robin():
    ...
    register_test_provider(registry, provider1)
    register_test_provider(registry, provider2)
    register_test_provider(registry, provider3)
