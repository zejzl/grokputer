...
# Config stub
CONFIG = {"auto_restart": True, "confirmation_timeout": 5.0}
...
async def run_poc_swarm():
    ...
    # Create agents
    observer = ObserverAgent("observer", message_bus, session_logger, CONFIG, action_executor)
    actor = ActorAgent("actor", message_bus, session_logger, CONFIG, action_executor)
    coordinator = Coordinator(message_bus, session_logger, CONFIG)
    ...
    async def coordinator_loop():
        ...
        try:
            ...
            task_msg = Message(
                ...
                content={"description": "capture screen and click at (100,100)", "task_id": "swarm_poc_task"},
                priority=MessagePriority.NORMAL,
            )
    ...
    try:
        await asyncio.gather(observer_loop(), actor_loop(), coordinator_loop(), return_exceptions=True)
    except Exception as e:
