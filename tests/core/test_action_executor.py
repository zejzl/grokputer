...
from src.core.action_executor import ActionExecutor, ActionPriority, Action, ActionHistory
...
async def test_single_click_action(executor, mock_pyautogui):
    ...
    result = await executor.execute_async(agent_id="test_agent", action={"type": "click", "x": 100, "y": 200})
...
async def test_type_action(executor, mock_pyautogui):
    ...
    result = await executor.execute_async(agent_id="test_agent", action={"type": "type", "text": "Hello World"})
...
async def test_key_press_action(executor, mock_pyautogui):
    ...
    result = await executor.execute_async(agent_id="test_agent", action={"type": "key", "key": "enter"})
...
async def test_statistics_tracking(executor, mock_pyautogui):
    ...
    await executor.execute_async(agent_id="agent1", action={"type": "click", "x": 10, "y": 10})

    await executor.execute_async(agent_id="agent1", action={"type": "type", "text": "test"})
