...
from src.config import requires_confirmation
...
class TestCoordinator:
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Test written for different Coordinator API")
    @pytest.mark.asyncio
    ...
    async def test_handle_response_aggregation(self, coordinator, mock_message_bus, mock_session_logger):
        ...
        message = {"type": "response", "task_id": task_id, "from": "observer", "content": {"success": True}}
    ...
    async def test_handle_error_recovery(self, coordinator, mock_message_bus, mock_session_logger):
        ...
        message = {"type": "error", "task_id": task_id, "from": "actor", "content": {"error": "Action failed"}}
    ...
    def test_requires_confirmation(self, coordinator):
        ...
        # Safe action
        assert not requires_confirmation("ls")
    ...
    async def test_stop_graceful_shutdown(self, coordinator, mock_session_logger):
        ...
        task_id = "test5"
        coordinator.active_tasks[task_id] = {"status": TaskStatus.DELEGATED, "description": "interrupted task"}
