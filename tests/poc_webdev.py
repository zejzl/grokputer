...
async def main():
    ...
    session_logger = SessionLogger(logs_dir=logs_dir, session_id=f"webdev_poc_{int(time.time())}")
    ...
    workflow_steps = [
        {
            ...
            "content": {
                "entities": [
                    {"name": "User", "fields": ["id", "email", "password_hash", "created_at", "is_active"]},
                    {"name": "Profile", "fields": ["id", "user_id", "first_name", "last_name", "bio"]},
                ],
                "relationships": [{"from": "Profile", "to": "User", "type": "one-to-one"}],
            },
        },
        {"name": "Setup JWT Authentication", "message_type": "setup_auth", "content": {"auth_type": "jwt"}},
        {
        ...
        },
        {
            ...
            "content": {"test_type": "unit", "coverage": 85},
        },
        {
...
async def create_user(user_data: dict):
    ...
    return user
        ...
        {"name": "Generate Docker Deployment", "message_type": "deploy_app", "content": {"platform": "docker"}},
    ]
