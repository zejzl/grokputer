...
class GrokAgent(BaseLLMAgent):
    ...
    def __init__(self, api_key: str, model: str = "grok-4-fast-reasoning", max_retries: int = 3, timeout: float = 30.0):
        """
        ...
        """
        super().__init__(role=AgentRole.GROK, model=model, api_key=api_key, max_retries=max_retries, timeout=timeout)

        self.client = AsyncOpenAI(api_key=api_key, base_url="https://api.x.ai/v1")

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10), reraise=True)
    async def call_api(self, messages: List[dict], max_tokens: int = 4096, temperature: float = 0.7, **kwargs) -> dict:
        """
        ...
        try:
            response = await asyncio.wait_for(
                self.client.chat.completions.create(
                    model=self.model, messages=messages, max_tokens=max_tokens, temperature=temperature, **kwargs
                ),
    ...
    async def generate_response(self, prompt: str, context: List[CollaborationMessage], round_number: int) -> str:
        """
        ...
        # Call API
        response = await self.call_api(messages=messages, max_tokens=2048, temperature=0.7)

        return response["content"]

    def _build_message_history(
        self, original_prompt: str, context: List[CollaborationMessage], round_number: int
    ) -> List[dict]:
