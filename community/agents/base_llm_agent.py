...
class BaseLLMAgent(ABC):
    ...
    def __init__(self, role: AgentRole, model: str, api_key: str, max_retries: int = 3, timeout: float = 30.0):
        """
    ...
    async def process_message(self, message: CollaborationMessage, original_prompt: str) -> CollaborationMessage:
        """
        ...
        Returns:
            Response message
        """
        logger.info(f"{self.role.value}: Processing message {message.message_id} " f"(round {message.round_number})")
        ...
        try:
            response_text = await self.generate_response(
                prompt=original_prompt, context=context, round_number=message.round_number
            )
