...
from src.autonomous import CodeScannerAgent, ProposalGeneratorAgent
from src.autonomous.models.findings import Finding, ScanReport
from src.autonomous.models.proposals import Proposal
from src.alerts import Notifier, AlertLevel, Alert
...
def _filter_findings(findings: List[Finding], category: str, severity: str) -> List[Finding]:
    ...
    if severity != "all":
        ...
        min_level_index = severity_levels.index(severity)
        filtered = [f for f in filtered if severity_levels.index(f.severity) >= min_level_index]

    return filtered


def _display_scan_report(report: ScanReport, filtered_findings: List[Finding]):
    ...
    summary_table.add_row("Target", report.scan_target)
    ...
    summary_table.add_row("Filtered Findings", str(len(filtered_findings)))

    console.print(summary_table)

    if filtered_findings:
        console.print(f"\n[bold red]Found {len(filtered_findings)} issue(s):[/bold red]")

        # Findings table
        findings_table = Table(title="Findings")
        findings_table.add_column("File", style="cyan")
        findings_table.add_column("Line", style="yellow")
        ...
        for finding in filtered_findings[:20]:  # Limit to first 20
            findings_table.add_row(
                str(finding.file_path) if finding.file_path else "N/A",
                str(finding.line_number or "N/A"),
                finding.category,
                finding.severity,
        ...
        if len(filtered_findings) > 20:
            console.print(f"[dim]... and {len(filtered_findings) - 20} more findings[/dim]")
    else:
...
@click.option(
    "--category",
    type=click.Choice(["security", "quality", "performance", "completeness", "architecture", "all"]),
    default="all",
...
async def _scan(
    ...
    with console.status("[bold green]Scanning code..."):
        if target_path.is_file():
            ...
            report = ScanReport(
                report_id=f"scan_{target_path.stem}", scan_target=str(target_path), findings=findings, files_scanned=1
            )
...
def _display_proposal(proposal: Proposal):
    """Display a proposal in a nice format."""
    console.print(Panel(f"[bold cyan]Proposal: {proposal.proposal_id}[/bold cyan]", expand=False))

    console.print(f"\n[bold]Description:[/bold]")
    console.print(Markdown(proposal.description))

    console.print(f"\n[bold]Changes:[/bold]")
    for i, change in enumerate(proposal.changes, 1):
        console.print(f"\n[cyan]Change {i}:[/cyan]")
        console.print(f"  File: {change.file_path}")
        console.print(f"  Type: {change.change_type}")

        if change.diff:
            console.print("\n  Diff:")
            syntax = Syntax(change.diff, "diff", theme="monokai", line_numbers=False)
            console.print(syntax)


async def _auto_propose(findings: List[Finding], dangerously_skip_permissions: bool):
    """Generate proposals for all findings."""
    ...
    console.print(f"\n[bold cyan]Generating {len(findings)} proposals...[/bold cyan]\n")

    for i, finding in enumerate(findings, 1):
        console.print(f"[dim]({i}/{len(findings)})[/dim] {finding.description[:80]}...")

        try:
            proposal = await proposer.generate_proposal(finding)
            console.print(f"[green]Generated proposal: {proposal.proposal_id}[/green]")
...
async def _improve(
    ...
    # Step 1: Scan
    await _scan(
        target, category, severity, None, auto_propose=True, dangerously_skip_permissions=dangerously_skip_permissions
    )


async def evolve_params(agent: str, chance: float, notifier=None):
    await asyncio.sleep(0.1)  # Yield for concurrency
    if random.random() < chance:
        # Mock param tweak logic with simulated improvement
        improvement = random.randint(10, 200)  # Simulate 10-200% improvement

        # Alert on high-impact evolutions (>50% improvement)
        if improvement > 50 and notifier:
        ...
        return f"{agent} evolved: +{improvement}% improvement"
    return f"{agent} stable"


async def security_scan(agent: str, target_path, scanner, notifier=None):
    vulns = []
    ...
    if vulns:
        ...
        if critical_count > 0 and notifier:
            await notifier.send(
                Alert(
                    title=f"{critical_count} Critical Security Issue{'s' if critical_count > 1 else ''} Detected",
                    message=(
                        f"Security scan found {critical_count} critical and "
                        f"{len(vulns)-critical_count} high severity issues in {target_path}.\n\n"
                        f"Top issues:\n" + "\n".join([f"• {v.description[:100]}" for v in vulns[:3]])
                    ),
                    level=AlertLevel.CRITICAL,
                    metadata={
                        "critical_count": critical_count,
                        "high_count": len(vulns) - critical_count,
...
async def async_daemon_cycle(target_path, agents: List[str], chance: float = 0.3, r=None, scanner=None, notifier=None):
    """Async param evolution + security scan — non-blocking swarm bliss."""
    tasks = []
    for agent in agents:
        tasks.append(asyncio.create_task(evolve_params(agent, chance, notifier)))
        tasks.append(asyncio.create_task(security_scan(agent, target_path, scanner, notifier)))
...
async def _daemon(target: str, interval: int, evolve_chance: float, no_redis: bool):
    ...
    if notifier.enable_email or notifier.enable_slack:
        console.print(f"[green]Alerts enabled:[/green] Email={notifier.enable_email}, Slack={notifier.enable_slack}")
    else:
        console.print("[dim]Alerts disabled (no credentials configured)[/dim]")
    ...
    if notifier.enable_email or notifier.enable_slack:
        await notifier.send(
            Alert(
                ...
                message=f"Autonomous monitoring daemon initialized.\n\nTarget: {target_path}\nInterval: {interval}s\nEvolution chance: {evolve_chance*100}%",
                level=AlertLevel.INFO,
                metadata={"target": str(target_path), "interval": f"{interval}s", "agents": ", ".join(agents)},
            )
        )
    ...
    try:
        ...
        if notifier.enable_email or notifier.enable_slack:
            await notifier.send(
                Alert(
                    title="Grokputer Daemon Stopped",
                    message=f"Daemon shut down gracefully after {cycle_count} cycles.",
                    level=AlertLevel.INFO,
                    metadata={"cycles_completed": cycle_count},
                )
            )
