...
class QwenBackend:
    def __init__(self, model_name: str = "Qwen/Qwen2.5-1.5B-Instruct", use_vision: bool = True):
        ...
        if use_vision:
            ...
            self.model = Qwen2VLForConditionalGeneration.from_pretrained(
                model_name.replace("-Instruct", "-VL"),
                torch_dtype=torch.float16 if self.device == "cuda" else torch.float32,
                device_map="auto",
            )
            self.vision_pipe = pipeline(
                "image-to-text", model=self.model, tokenizer=self.tokenizer, processor=self.processor
            )
        else:
    ...
    def apply_chat_template(self, messages: List[Dict[str, str]]) -> str:
        ...
        formatted = self.tokenizer.apply_chat_template(
            messages + [{"role": "assistant", "content": ""}], tokenize=False, add_generation_prompt=True
        )
        return formatted
    ...
    def generate(self, prompt: str, tools: Optional[List[Dict]] = None, max_tokens: int = 512) -> Dict[str, Any]:
        ...
        return {"choices": [{"message": {"role": "assistant", "content": response}}], "tool_calls": tool_calls}
