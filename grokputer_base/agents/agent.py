...
class Agent:
    ...
    async def _agent_loop(self, user_input: str) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            response = self.client.messages.create(**params, extra_headers=merged_headers)
            ...
            if self.verbose:
                for block in response.content:
                    if block.type == "text":
                        ...
                        print(f"\n[{self.name}] Tool call: " f"{block.name}({params_str})")
            ...
            if tool_calls:
                ...
                if self.verbose:
                    for block in tool_results:
                        print(f"\n[{self.name}] Tool result: " f"{block.get('content')}")
