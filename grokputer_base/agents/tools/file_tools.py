        ...
        try:
            ...
            def write_sync():
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                return f"Successfully wrote {len(content)} " f"characters to {path}"
            ...
            def edit_sync():
                try:
                    ...
                    if old_text not in content:
                        return f"Error: The specified text was not " f"found in {path}"
                    ...
                    if count > 1:
                        ...
                        with open(file_path, "w", encoding="utf-8") as f:
                            f.write(new_content)
                        return f"Warning: Found {count} occurrences. " f"All were replaced in {path}"
                    else:
