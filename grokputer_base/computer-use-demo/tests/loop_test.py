...
async def test_loop():
    ...
    client.beta.messages.with_raw_response.create.return_value.parse.side_effect = [
        mock.Mock(
            ...
            content=[
                ...
                ToolUseBlock(type="tool_use", id="1", name="computer", input={"action": "test"}),
            ],
        ),
    ...
    tool_collection.run.return_value = mock.Mock(output="Tool output", error=None, base64_image=None)

    output_callback = mock.Mock()
    tool_output_callback = mock.Mock()
    api_response_callback = mock.Mock()

    with mock.patch("computer_use_demo.loop.Anthropic", return_value=client), mock.patch(
        ...
        tool_collection.run.assert_called_once_with(name="computer", tool_input={"action": "test"})
        output_callback.assert_called_with(BetaTextBlockParam(text="Done!", type="text", citations=None))
