...
WARNING_TEXT = "[WARNING] Security Alert: Never provide access to sensitive accounts or data, as malicious web content can hijack Claude's behavior"
...
def _reset_model_conf():
    model_conf = MODEL_TO_MODEL_CONF.get(st.session_state.model, SONNET_3_5_NEW)  # Default fallback
...
async def main():
    ...
    with st.sidebar:
        ...
        st.radio(
            ...
            on_change=lambda: setattr(st.session_state, "tool_version", st.session_state.tool_versions),
        )
