...
"""
            ...
            for item in data:
                output += (
                    f"- {item['agent']}: Rolls={item['rolls']}, Avg={item['avg_total']:.2f}, Max={item['max_total']}\n"
                )
            return output
    ...
    try:
        if messagebus:
            ...
            coordinator = CollaborationCoordinator(
                grok_client=grok_client, session_logger=session_logger, session_id=session_id
            )

            result = asyncio.run(coordinator.run_collaboration(task=task, max_rounds=max_rounds))
...
"""
    ...
    for iteration in range(max_iterations):
        ...
        response = await grok_client.create_message(
            system_prompt=system_prompt, user_prompt=user_prompt, tools=config.TOOLS
        )
