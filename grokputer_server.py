...
async def scan_vault(vault_path: str = "vault") -> Dict[str, Any]:
    ...
    try:
        ...
        if not vault_dir.exists():
            return {"success": False, "error": f"Vault directory not found: {vault_path}", "files": []}
        ...
        for pattern in ["**/*.pdf", "**/*.md", "**/*.txt", "**/*.json"]:
            for file_path in vault_dir.glob(pattern):
                if file_path.is_file():
                    stat = file_path.stat()
                    files.append(
                        {"path": str(file_path.relative_to(vault_dir)), "size": stat.st_size, "modified": stat.st_mtime}
                    )
...
async def invoke_prayer(prayer_type: str = "server_prayer") -> Dict[str, str]:
    ...
    try:
        ...
        if not prayer_file.exists():
            return {"success": False, "error": "server_prayer.txt not found", "prayer": ""}
        ...
        return {"success": False, "error": str(e), "prayer": ""}
...
async def get_vault_stats(vault_path: str = "vault") -> Dict[str, Any]:
    ...
    try:
        ...
        if not vault_dir.exists():
            return {"success": False, "error": f"Vault directory not found: {vault_path}", "stats": {}}
        ...
        return {"success": True, "vault_path": str(vault_dir), "stats": stats}
...
async def execute_bash_safe(command: str, require_confirm: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": "Command timeout (30s)", "command": command}
    except Exception as e:
        return {"success": False, "error": str(e), "command": command}


@mcp.tool()
async def capture_screenshot_region(left: int, top: int, width: int, height: int) -> Dict[str, Any]:
    """
    ...
    if not HAS_PYAUTOGUI:
        return {"success": False, "error": "pyautogui not installed. Install with: pip install pyautogui pillow"}
...
async def list_recent_sessions(limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        if not logs_dir.exists():
            return {"success": True, "sessions": [], "message": "No logs directory found"}
            ...
            if session_dir.is_dir():
                ...
                if session_json.exists():
                    try:
                        ...
                        sessions.append({"session_id": session_dir.name, "error": "Could not parse session.json"})
