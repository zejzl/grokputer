...
async def _run_collaboration_mode(task: str, max_rounds: int, debug: bool, review_mode: bool):
    ...
    # Import here to avoid hard dependency on anthropic at startup
...
""")
        ...
        if self.grok_client.test_connection():
            ...
            raise ConnectionError("Failed to connect to Grok")
...
""")
    ...
    while True:  # Replace recursive call with while loop
...
async def _run_swarm_mode(task: str, agent_roles: list, debug: bool):
    ...
    try:
        ...
        logger.warning("[SWARM] Execution timed out after 5 minutes")
        print("\n[TIMEOUT] Swarm execution exceeded 5 minutes\n")
    except Exception as e:
...
def main():
    ...
    Wrapped in try/except to ensure errors are visible on Windows.
    """
    load_dotenv()
    ...
    skip_boot = os.getenv("SKIP_BOOT", "false").lower() == "true"
...
if __name__ == "__main__":
    try:
        ...
        # Print full traceback so the error is visible
        import traceback
        print("\n" + "=" * 70)
