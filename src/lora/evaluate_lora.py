...
def evaluate_lora(adapter_path="./lora_adapter", model_name="meta-llama/Llama-2-7b-hf"):
    ...
    def compute_perplexity(model, tokenizer, dataset, batch_size=4):
        ...
        dataloader = DataLoader(
            dataset, batch_size=batch_size, collate_fn=lambda x: tokenizer.pad(x, return_tensors="pt")
        )
    ...
    generator = pipeline(
        "text-generation", model=merged_model, tokenizer=tokenizer, device=0 if torch.cuda.is_available() else -1
    )
    base_generator = pipeline(
        "text-generation", model=base_model, tokenizer=tokenizer, device=0 if torch.cuda.is_available() else -1
    )
