...
class ActionExecutor:
            ...
            try:
                ...
                try:
                    ...
                    if action.agent_id in self.result_queues:
                        self.result_queues[action.agent_id].put((action.request_id, result))

                except Exception as e:
                    logger.error(
                        f"[ActionExecutor] Action failed: {action.action_type} " f"from {action.agent_id}: {e}"
                    )
    ...
    def _execute_action(self, action: Action) -> Dict[str, Any]:
        ...
        if action_type == "click":
            ...
            return {"status": "success", "action": "click", "coords": (x, y), "button": button, "clicks": clicks}
            ...
            return {"status": "success", "action": "screenshot", "data": img_str, "dimensions": screenshot.size}
    ...
    def _add_to_history(self, action: Action, result: Dict, execution_time: float):
        ...
        history_entry = ActionHistory(action=action, result=result, execution_time=execution_time)
    ...
    async def execute_async(
        ...
        try:
            result = await asyncio.wait_for(
                asyncio.to_thread(self._wait_for_result, agent_id, request_id), timeout=timeout
            )
            return result
        except asyncio.TimeoutError:
            logger.warning(
                f"[ActionExecutor] Action timeout: {action_obj.action_type} " f"from {agent_id} after {timeout}s"
            )
    ...
    async def execute_batch_async(
        ...
        for i, action in enumerate(actions):
            logger.debug(f"[ActionExecutor] Batch {i+1}/{len(actions)}: " f"{action.get('type')} for {agent_id}")

            result = await self.execute_async(agent_id=agent_id, action=action, priority=priority, timeout=timeout)
            ...
            if stop_on_error and result.get("status") != "success":
                logger.warning(
                    f"[ActionExecutor] Batch stopped at {i+1}/{len(actions)} " f"due to error: {result.get('error')}"
                )
                break
