...
def create_workflow_graph(task: str) -> StateGraph:
    ...
    workflow.add_conditional_edges("coder", lambda s: "tester" if s.get("code_actions") else END, {"tester": "tester"})
    workflow.add_conditional_edges(
        "tester", lambda s: "coder" if not s.get("converged", True) else END, {"coder": "coder", END: END}
    )
