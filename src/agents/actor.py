...
class Actor(BaseAgent):
    ...
    async def _handle_subtask(self, message: Message) -> Dict:
        ...
        if action_type == "perform_action":
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": f"Unknown action type: {action_type}"},
    ...
    async def _execute_generic_action(self, task_id: str, params: Dict) -> Dict:
        ...
        if not action:
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": "Could not parse command"},
                "priority": MessagePriority.NORMAL,
            }
        ...
        try:
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": str(e)},
    ...
    async def _execute_with_retry(self, action: Dict) -> Dict:
        ...
        return await self.action_executor.execute_async(
            agent_id=self.agent_id, action=action, priority=ActionPriority.NORMAL, timeout=10.0
        )
    ...
    async def _execute_bash(self, task_id: str, params: Dict) -> Dict:
        ...
        try:
            result = await asyncio.wait_for(
                asyncio.to_thread(self._run_bash_command, command), timeout=params.get("timeout", 30.0)
            )
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": "Command timeout"},
                "priority": MessagePriority.HIGH,
            }
    ...
    def _run_bash_command(self, command: str) -> Dict:
        ...
        try:
            ...
            result = subprocess.run(args, shell=False, capture_output=True, text=True, timeout=30.0)
            ...
            return {"returncode": -1, "stdout": "", "stderr": "Command timeout", "command": command}
        except Exception as e:
            return {"returncode": -1, "stdout": "", "stderr": str(e), "command": command}
    ...
    async def _execute_file_op(self, task_id: str, params: Dict) -> Dict:
        ...
        if not self._is_safe_file_path(file_path):
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": "Path validation failed"},
                "priority": MessagePriority.HIGH,
            }
        ...
        try:
            ...
            self.session_logger.log_tool_execution(tool_name="file", params=params, result=result, status="success")

            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "success", "result": result},
                "priority": MessagePriority.NORMAL,
            }
