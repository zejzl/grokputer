...
class Taskmaster(BaseAgent):
    ...
    async def suggest_tasks(self, current_state: str = "") -> List[TaskSuggestion]:
        ...
        messages = [
            ...
            {"role": "user", "content": f"Metrics: {insights}. Current state: {current_state}. Suggest 3-5 tasks with priority, impact (0-10), effort (low/medium/high), category (integration/feature/opt). Format as JSON list."}
        ]
    ...
    async def get_next_step(self, task: str) -> str:
        ...
        messages = [
            {"role": "user", "content": f"For task '{task}', provide step-by-step implementation plan."}
        ]
