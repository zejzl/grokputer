...
class ScreenshotCache:
    ...
    def put(self, screenshot_hash: str, screenshot_b64: str, analysis: Dict[str, Any]):
        ...
        self.cache[screenshot_hash] = {"screenshot": screenshot_b64, "analysis": analysis, "timestamp": time.time()}
...
class Observer(BaseAgent):
    ...
    async def _handle_subtask(self, message: Message) -> Dict:
        ...
        if action == "capture_screen":
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": f"Unknown action: {action}"},
    ...
    async def _capture_screenshot(self, region: Optional[Tuple[int, int, int, int]] = None) -> str:
        ...
        screenshot_b64 = await asyncio.to_thread(self.screen_observer.screenshot_to_base64, region=region, format="PNG")
    ...
    async def _analyze_screenshot(self, screenshot_b64: str) -> Dict[str, Any]:
        ...
        response = await asyncio.to_thread(
            self.grok_client.create_message, task=prompt, screenshot_base64=screenshot_b64
        )
        ...
        if response.get("status") == "success":
            ...
            return {"error": error, "timestamp": time.time()}
    ...
    async def _handle_analysis(self, message: Message) -> Dict:
        ...
        if not screenshot_b64:
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "error", "error": "No screenshot provided"},
                "priority": MessagePriority.NORMAL,
            }
        ...
        try:
            ...
            return {
                "to": "coordinator",
                "type": "response",
                "content": {"task_id": task_id, "status": "success", "result": {"analysis": analysis}},
                "priority": MessagePriority.NORMAL,
            }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_capture_time_ms": (
                int(self.stats["total_capture_time"] / total_captures * 1000) if total_captures > 0 else 0
            ),
            "avg_analysis_time_ms": (
                int(self.stats["total_analysis_time"] / self.stats["grok_calls"] * 1000)
                if self.stats["grok_calls"] > 0
                else 0
            ),
