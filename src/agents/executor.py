...
class ExecutorAgent(Actor):
    ...
    def __init__(
        ...
        super().__init__(message_bus, session_logger, config_dict, action_executor, heartbeat_interval)
        ...
        self.execution_queue: Dict[str, List[Dict]] = {"critical": [], "high": [], "normal": [], "low": []}
        ...
        self.session_logger.log_agent_init(
            self.agent_id, f"Executor ready (circuit_breakers={self.enable_circuit_breakers})"
        )
    ...
    async def _execute_with_retry(self, content: Dict) -> Dict:
        ...
        while retry_count <= max_retries:
            try:
                ...
                return {"status": "success", "result": asdict(exec_result)}
                ...
                if retry_count <= max_retries:
                    ...
                    self.session_logger.log_agent_activity(
                        self.agent_id, f"Retry {retry_count}/{max_retries} after {delay:.2f}s: {operation_name}"
                    )
                    await asyncio.sleep(delay)

        # All retries exhausted - try fallback
        if fallback:
            self.session_logger.log_agent_activity(self.agent_id, f"Attempting fallback for {operation_name}")
            try:
        ...
        return {"status": "failed", "result": asdict(exec_result)}
    ...
    async def _execute_action(self, action: Dict) -> Any:
        ...
        message = Message(
            ...
            content={"action": action_type, "params": params, "task_id": action.get("id", "unknown")},
        )
        ...
        if operation_name in self.circuit_breakers:
            ...
            if circuit.state == CircuitState.HALF_OPEN:
                ...
                if circuit.success_count >= circuit.half_open_max_attempts:
                    circuit.state = CircuitState.CLOSED
                    circuit.failure_count = 0
                    self.session_logger.log_agent_activity(self.agent_id, f"Circuit recovered: {operation_name}")
    ...
    async def _record_failure(self, operation_name: str):
        ...
        if circuit.state == CircuitState.CLOSED:
            if circuit.failure_count >= circuit.threshold:
                ...
                self.session_logger.log_agent_activity(
                    self.agent_id, f"Circuit opened: {operation_name} ({circuit.failure_count} failures)"
                )
    ...
    async def _execute_batch(self, content: Dict) -> Dict:
        ...
        if parallel:
            # Execute in parallel
            tasks = [self._execute_with_retry({"action": action}) for action in actions]
    ...
    async def _execute_with_dependencies(self, actions: List[Dict], dependencies: Dict) -> Dict:
        ...
        while len(completed) < len(actions):
            ...
            ready = [
                ...
                and all(dep in completed for dep in dependencies.get(action.get("id"), []))
            ]
            ...
            tasks = [self._execute_with_retry({"action": action}) for action in ready]
    ...
    def _get_circuit_status(self, content: Dict) -> Dict:
        ...
        if operation_filter:
            if operation_filter in self.circuit_breakers:
                ...
                return {"status": "success", "circuit": asdict(circuit)}
            else:
                return {"status": "not_found", "operation": operation_filter}
        else:
            return {
                "status": "success",
                "circuits": {name: asdict(circuit) for name, circuit in self.circuit_breakers.items()},
            }
    ...
    def _reset_circuit(self, content: Dict) -> Dict:
        ...
        if operation in self.circuit_breakers:
            ...
            self.session_logger.log_agent_activity(self.agent_id, f"Circuit reset: {operation}")
    ...
    def _get_executor_stats(self) -> Dict:
        ...
        return {
            ...
            "open_circuits": sum(1 for c in self.circuit_breakers.values() if c.state == CircuitState.OPEN),
    ...
    async def on_start(self):
        ...
        self.session_logger.log_agent_ready(
            self.agent_id, "Executor active - advanced retry and circuit breaker enabled"
        )
