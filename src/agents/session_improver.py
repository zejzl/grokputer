...
class SessionImprover:
    ...
    def analyze_session(self, session_id: str) -> Dict:
        ...
        if iterations:
            ...
            analysis["metrics"]["avg_iteration_time"] = sum(it.get("duration", 0) for it in iterations) / len(
                iterations
            )
            ...
            if failed:
                ...
                analysis["recommendations"].append("Add retry logic for failed iterations")
            else:
        ...
        if tool_calls:
            ...
            if repeated:
                ...
                analysis["recommendations"].append(f"Cache results for repeated tools: {', '.join(repeated.keys())}")
        ...
        if errors:
            ...
            analysis["recommendations"].append(f"Add error handling for: {', '.join(error_types.keys())}")
        else:
        ...
        if "api_calls" in data:
            ...
            if total_cost > 1.0:
                analysis["recommendations"].append("High API cost - consider caching or reducing iterations")
        ...
        if iterations:
            ...
            if slow_iterations:
                ...
                analysis["recommendations"].append("Optimize slow operations - consider async processing")

        # Check task completion
        if data.get("status") == "completed":
            analysis["strengths"].append("Task completed successfully")
        elif data.get("status") == "partial":
            analysis["recommendations"].append("Task incomplete - increase max_iterations or refine prompt")
