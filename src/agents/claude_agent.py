...
class ClaudeAgent(BaseLLMAgent):
    ...
    def __init__(
        self, api_key: str, model: str = "claude-sonnet-4-5-20250929", max_retries: int = 3, timeout: float = 30.0
    ):
        """
        ...
        """
        super().__init__(role=AgentRole.CLAUDE, model=model, api_key=api_key, max_retries=max_retries, timeout=timeout)
    ...
    async def generate_response(self, prompt: str, context: List[CollaborationMessage], round_number: int) -> str:
        ...
        # Call API
        response = await self.call_api(messages=messages, max_tokens=2048, temperature=0.7, system=system_prompt)
