...
class AnalyzerAgent(CognitiveAnalyzerMixin, BaseAgent):
    """
    ...
    def __init__(
        ...
        # Initialize cognitive capabilities first
        CognitiveAnalyzerMixin.__init__(self, cognitive_enabled=cognitive_enabled)
        # Then initialize base agent
        BaseAgent.__init__(self, agent_id, message_bus, session_logger, config, heartbeat_interval)
    ...
    async def _record_metric(self, content: Dict) -> Dict:
        ...
        return {"status": "recorded", "metric_name": metric.metric_name, "value": metric.value}
    ...
    async def _record_task_performance(self, content: Dict) -> Dict:
        ...
        return {"status": "recorded", "task_type": task_type, "execution_time": execution_time}
    ...
    async def _get_health_status(self, content: Dict) -> Dict:
        ...
        for agent_id, health in self.agent_health.items():
            ...
            agent_tasks = [
                k
                for k in self.task_performance.keys()
    ...
    async def _detect_bottlenecks(self) -> Dict:
        """Detect performance bottlenecks using cognitive enhancement."""
        bottlenecks = []
        current_time = time.time()

        # Gather current performance data for cognitive analysis
        performance_data = {
            "task_performance": dict(self.task_performance),
            "agent_health": {aid: asdict(health) for aid, health in self.agent_health.items()},
            "recent_metrics": list(self.metrics.get("response_time", []))[-20:],
            "timestamp": current_time,
        }

        # Use cognitive enhancement for bottleneck detection
        cognitive_analysis = self.enhance_performance_analysis(
            performance_data, historical_data=self._get_performance_history()
        )

        # Extract cognitive insights
        anomaly_detection = cognitive_analysis.get("anomaly_detection", [])
        trend_analysis = cognitive_analysis.get("trend_analysis", {})

        # Log cognitive insights
        if anomaly_detection:
            logger.info(f"Cognitive anomaly detection: {anomaly_detection}")
        if trend_analysis.get("trend_detection"):
            logger.info(f"Performance trend detected: {trend_analysis}")

        # 1. Enhanced slow task detection with cognitive insights
    ...
    async def _generate_report(self, content: Dict) -> Dict:
        ...
        report["summary"]["critical_bottlenecks"] = sum(1 for b in self.bottlenecks if b.severity == "critical")
        ...
        if report_type == "detailed":
            report["detailed"] = {
                ...
                "top_slow_tasks": sorted(task_summary.items(), key=lambda x: x[1]["avg_execution_time"], reverse=True)[
                    :5
                ],
            }
        ...
        self.session_logger.log_agent_activity(self.agent_id, f"Generated {report_type} report covering {time_range}s")
    ...
    def _get_performance_history(self) -> List[Dict[str, Any]]:
        """Get recent performance history for cognitive analysis."""
        history = []

        # Get recent task performance data
        for task_type, times in self.task_performance.items():
        ...
        return history[-20:]  # Return last 20 historical records
    ...
    async def on_start(self):
        ...
        self.session_logger.log_agent_ready(self.agent_id, "Analyzer monitoring active")
