...
class ConsensusDetector:
    ...
    def __init__(self, convergence_threshold: float = 0.6):
        ...
        self.agreement_regex = [re.compile(pattern, re.IGNORECASE) for pattern in self.AGREEMENT_PATTERNS]
        self.disagreement_regex = [re.compile(pattern, re.IGNORECASE) for pattern in self.DISAGREEMENT_PATTERNS]
    ...
    def analyze_round(self, messages: List[CollaborationMessage], round_number: int) -> ConsensusSignal:
        ...
        confidence = self._calculate_confidence(len(agreement_found), len(disagreement_found), convergence)
    ...
    def _calculate_confidence(self, agreement_count: int, disagreement_count: int, convergence: float) -> float:
        """
        ...
        confidence = 0.4 * agreement_score + 0.3 * disagreement_penalty + 0.3 * convergence
