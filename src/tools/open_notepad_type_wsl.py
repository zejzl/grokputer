...
subprocess.run(['powershell.exe', '-Command', 'Start-Process notepad.exe'], shell=False)
time.sleep(3)  # Wait for focus

# Type text using PowerShell SendKeys (Windows .NET)
subprocess.run(['powershell.exe', '-Command', '[System.Windows.Forms.SendKeys]::SendWait(\"hello word, why?\")'], shell=False)
