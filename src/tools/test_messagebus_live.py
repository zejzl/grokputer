...
async def test_priority_ordering(bus: MessageBus):
    ...
    if msg1.priority == MessagePriority.HIGH and msg2.priority == MessagePriority.NORMAL and msg3.priority == MessagePriority.LOW:
...
async def main():
    ...
    await asyncio.gather(
        claude_agent(bus, message_for_grok),
        grok_agent(bus)
    )
    ...
    if stats['latency_by_type']:
        ...
        for msg_type, latency in stats['latency_by_type'].items():
            print(f"  - {msg_type}: {latency['avg_ms']:.2f}ms avg "
