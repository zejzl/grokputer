...
class ScreenObserver:
    ...
    def __init__(self, quality: int = None, max_size: Tuple[int, int] = None):
        """
        ...
        logger.info(
            f"Screen observer initialized: quality={self.quality}, " f"max_size={self.max_width}x{self.max_height}"
        )

    async def capture_screenshot(self, region: Optional[Tuple[int, int, int, int]] = None) -> Image.Image:
        """
    ...
    async def save_screenshot(self, filepath: str, region: Optional[Tuple[int, int, int, int]] = None) -> bool:
        """
    ...
    def locate_on_screen(self, template_path: str, confidence: float = 0.8) -> Optional[Tuple[int, int, int, int]]:
        """
