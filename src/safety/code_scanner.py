...
class CodeSecurityScanner:
    ...
    # Unsafe imports
    UNSAFE_IMPORTS = {
        'pickle', 'shelve', 'marshal', 'yaml',  # Can lead to code execution
    ...
    def scan_directory(self, dir_path: str, exclude_patterns: List[str] = None) -> Dict[str, List[SecurityVulnerability]]:
        """
        ...
        for alias in node.names:
            if alias.name in CodeSecurityScanner.UNSAFE_IMPORTS:
                self.scanner._add_vulnerability(
                    ...
                    "MEDIUM",
                    node,
                    f"Import of potentially unsafe module '{alias.name}'"
                )
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        if node.module in CodeSecurityScanner.UNSAFE_IMPORTS:
            self.scanner._add_vulnerability(
                ...
                "MEDIUM",
                node,
                f"Import from potentially unsafe module '{node.module}'"
            )
        self.generic_visit(node)
    ...
    def _check_shell_injection(self, node: ast.Call):
        ...
        if shell_true:
            ...
            self.scanner._add_vulnerability(
                ...
                "HIGH",
                node,
                "Use of shell=True increases command injection risk"
            )
...
def scan_project_for_vulnerabilities(project_path: str = ".", output_file: str = None) -> Dict[str, List[SecurityVulnerability]]:
    """
