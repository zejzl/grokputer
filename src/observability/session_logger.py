...
class SessionLogger:
    ...
    def __init__(self, session_id: str, task: str, log_dir: Path, swarm_mode: bool = False):
        """
    ...
    def log_agent_activity(self, agent_id: str, state: str, data=None):
    ...
    def log_api_call(self, duration: float, model: str, success: bool):
        """Log API call."""
        call = {"duration": duration, "model": model, "success": success, "timestamp": time.time()}
    ...
    def log_session_end(self):
        """End session logging (alias for finalize for backward compatibility)."""
        self.finalize()
